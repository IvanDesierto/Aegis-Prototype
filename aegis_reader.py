...
def get_hardware_key(MAC):
    byte_key = str(MAC).encode() #Turn MAC address (int) into string, then convert to byte
    hash_digest = hashlib.sha256(byte_key).digest() #byte is hashed
    return base64.urlsafe_b64encode(hash_digest) #hashed byte is converted to be urlsafe
...
def self_destruct(file_name):
    ...
    try:
        with open(file_name, "wb") as f:
            f.write(os.urandom(1024)) #release the gibberish

        os.remove(file_name)
        #File is overwritten, then deleted for extra safety
